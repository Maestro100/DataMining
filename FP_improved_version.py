...
"Simple function to generate subs of a list"
...
itemsets = itemsets_from_file("forests.txt")                           
minsup = 150
fis = [itemset for itemset in maximal_frequent_itemsets(itemsets, minsup)]
fis.sort(key=len)
final_list= []
final_set = set()
...
final_set = sorted(set(map(tuple, final_list)), reverse=True)
final_set.sort(key=len)
